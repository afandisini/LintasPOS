(function () {
  ...
  function getToggleOrigin(event) {
    var target =
      event && event.currentTarget && event.currentTarget.getBoundingClientRect
        ? event.currentTarget
  ...
  function revealTheme(theme, event, done) {
    ...
    overlay.style.transition =
      "clip-path 560ms cubic-bezier(0.22, 1, 0.36, 1), -webkit-clip-path 560ms cubic-bezier(0.22, 1, 0.36, 1), opacity 200ms ease-out";
  ...
  function normalizeLabel(label) {
    ...
    if (typeof label === "string") {
      var text = label.trim();
      if (/^\d+$/.test(text)) {
        var monthIndex = parseInt(text, 10);
    ...
    if (typeof label === "object") {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (
          Object.prototype.hasOwnProperty.call(label, keys[i]) &&
          label[keys[i]] !== null &&
          label[keys[i]] !== undefined &&
          String(label[keys[i]]).trim() !== ""
        ) {
          return normalizeLabel(label[keys[i]]);
        }
      }
    }
    return "";
  ...
  labels = Array.isArray(labels) ? labels.map(normalizeLabel) : [];
  ...
  function formatCompactCurrency(value) {
    ...
    if (abs >= 1e12) {
      result =
        (abs / 1e12).toFixed(abs >= 1e13 ? 0 : 1).replace(/\.0$/, "") + "T";
    } else if (abs >= 1e9) {
      result =
        (abs / 1e9).toFixed(abs >= 1e10 ? 0 : 1).replace(/\.0$/, "") + "M";
    } else if (abs >= 1e6) {
      result =
        (abs / 1e6).toFixed(abs >= 1e7 ? 0 : 1).replace(/\.0$/, "") + "M";
    } else if (abs >= 1e3) {
      result =
        (abs / 1e3).toFixed(abs >= 1e4 ? 0 : 1).replace(/\.0$/, "") + "K";
    } else {
      result = String(Math.round(abs));
    }

    return "Rp " + sign + result;
  }
          ...
          tooltip: {
            callbacks: {
              ...
              label: function (context) {
                var index = context && Number.isInteger(context.dataIndex)
                  ? context.dataIndex
                  : -1;
                var monthLabel = index >= 0 ? labelMap[index] || "" : "";
                var value = context && context.parsed ? context.parsed.y : 0;
                var amount = formatCompactCurrency(value);
                return monthLabel ? monthLabel + " " + amount : amount;
              },
            },
          },
        },
...
(function () {
  ...
  var iconClass = {
    barang: "",
    jasa: "gs-icon-jasa",
    pelanggan: "gs-icon-pelanggan",
    supplier: "gs-icon-supplier",
  };

  function open() {
    dropdown.classList.add("gs-open");
  }
  function close() {
    dropdown.classList.remove("gs-open");
    activeIdx = -1;
  }
  ...
  function render(results) {
    if (!results.length) {
      dropdown.innerHTML =
        '<div class="gs-empty">Tidak ada hasil ditemukan.</div>';
      open();
      return;
    }
    ...
    Object.keys(grouped).forEach(function (type) {
      html +=
        '<div class="gs-group-label">' + (typeLabel[type] || type) + "</div>";
      grouped[type].forEach(function (r) {
        html +=
          '<a class="gs-item" href="' +
          r.url +
          '">' +
          ...
          '<div class="gs-item-label">' +
          esc(r.label) +
          "</div>" +
          (r.sub ? '<div class="gs-item-meta">' + esc(r.sub) + "</div>" : "") +
          (r.meta
            ? '<div class="gs-item-meta">' + esc(r.meta) + "</div>"
            : "") +
          "</div></a>";
      });
    });
  ...
  function search(q) {
    dropdown.innerHTML =
      '<div class="gs-loading"><i class="bi bi-arrow-repeat spin-icon"></i> Mencari...</div>';
    open();
    fetch("/api/search?q=" + encodeURIComponent(q))
      ...
      .catch(function () {
        dropdown.innerHTML =
          '<div class="gs-empty">Gagal memuat hasil pencarian.</div>';
      });
  }
