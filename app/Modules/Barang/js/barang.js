(function () {
  ...
  function openModal(id) {
    ...
    if (typeof window.csEnhance === 'function') window.csEnhance(modal);
  }
  ...
  function looksLikeImageFieldName(name) {
    return /(gambar|image|img|foto|photo|thumbnail|thumb|avatar|logo)/i.test(
      String(name || ""),
    );
  }
  ...
  function createImageFallbackHtml(imageUrl, initials, alt, cssClass) {
    ...
    var fallback =
      '<span class="generated-avatar-fallback" aria-hidden="true">' +
      safeInitials +
      "</span>";
  ...
  function renderTableCellValue(row, fieldName, helperHtml) {
    if (helperHtml !== "") {
      if (looksLikeImageFieldName(fieldName)) {
        var preview = resolveImagePreview(
          row,
          window.generatedCrudConfig || {},
        );
        var fallbackClass =
          "generated-img-cell generated-img-cell-" +
          slugify(fieldName || "image");
        return createImageFallbackHtml(
          preview.url,
          preview.initials,
          preview.alt,
          fallbackClass,
    ...
    var value = getDisplayValue(row, fieldName);
    if (looksLikeImageFieldName(fieldName) && looksLikeImageUrl(value)) {
      var displayName = getRowDisplayName(row);
      return createImageFallbackHtml(
        String(value).trim(),
  ...
  function buildActionButtons(row, cfg) {
    ...
    return (
      ...
      '<button type="button" class="btn-g btn-sm btn-generated-detail" data-id="' +
      id +
      '" title="Detail">' +
    ...
    displayColumns.forEach(function (column) {
      ...
      columns.push({
        ...
        render: function (data, type, row) {
          ...
          return renderTableCellValue(row, name, helperHtml);
        },
      });
    });
    ...
    document.addEventListener("click", function (event) {
      ...
      Object.keys(row).forEach(function (key) {
        ...
        if (
          ...
          var currentEl = document.getElementById("edit_current_" + key);
  ...
  function bindDetailButtons() {
    ...
    document.addEventListener("click", function (event) {
      var button =
        event.target instanceof Element
          ? event.target.closest(".btn-generated-detail")
          : null;
      if (!(button instanceof HTMLElement)) return;
      ...
      if (contentEl) {
        ...
        html +=
          '<div class="panel-head"><span class="panel-title">Gambar</span></div>';
        html += '<div class="panel-body">';
        html += createImageFallbackHtml(
          preview.url,
          preview.initials,
          preview.alt,
          "generated-detail-preview",
        );
        html += "</div></div>";
        ...
        html +=
          '<div class="panel-body"><div class="table-responsive"><table class="dtable table-sm align-middle mb-0 generated-detail-table"><tbody>';
        var displayColumns = Array.isArray(cfg.displayColumns)
          ? cfg.displayColumns
          : [];
        displayColumns.forEach(function (column) {
          ...
          var displayValue =
            helperHtml !== ""
              ? helperHtml
              : escapeHtml(safeString(getDisplayValue(row, name)));
          html += "<tr>";
  ...
  function ensureImageZoomModal() {
    var existing = document.getElementById("cmImageZoomGenerated");
    if (existing) return existing;

    if (!document.getElementById("generatedImageZoomStyle")) {
      var style = document.createElement("style");
      style.id = "generatedImageZoomStyle";
      style.textContent =
        "#cmImageZoomGenerated{padding:16px;}" +
        ".generated-image-zoom-box{position:relative;width:min(96vw,980px);max-height:92vh;padding:10px;overflow:hidden;}" +
        ".generated-image-zoom-close{position:absolute;top:10px;right:10px;z-index:3;}" +
        ".generated-image-zoom-wrap{display:flex;align-items:center;justify-content:center;max-height:calc(92vh - 20px);overflow:auto;border-radius:12px;}" +
        ".generated-image-zoom-wrap img{max-width:100%;max-height:88vh;object-fit:contain;display:block;border-radius:10px;}" +
        "@media (max-width: 576px){.generated-image-zoom-box{width:calc(100vw - 20px);padding:8px;}.generated-image-zoom-wrap img{max-height:84vh;}}";
      document.head.appendChild(style);
    }

    var modal = document.createElement("div");
    modal.className = "cm-bg";
    modal.id = "cmImageZoomGenerated";
    modal.setAttribute("data-cm-bg", "");
    modal.innerHTML =
      '<div class="panel cm-box cm-box-lg generated-image-zoom-box" role="dialog" aria-modal="true" aria-label="Zoom gambar">' +
      '<button type="button" class="cm-x generated-image-zoom-close" data-cm-close aria-label="Close"><i class="bi bi-x-lg"></i></button>' +
      '<div class="generated-image-zoom-wrap">' +
      '<img id="generatedImageZoomPreview" src="" alt="Preview gambar">' +
      "</div>" +
      "</div>";
  ...
  function bindImageZoom() {
    document.addEventListener("click", function (event) {
      var img =
        event.target instanceof Element
          ? event.target.closest(".generated-img-cell img")
          : null;
      if (!(img instanceof HTMLImageElement)) return;
      var src = String(img.getAttribute("src") || "").trim();
      if (src === "") return;

      var modal = ensureImageZoomModal();
      var preview = modal.querySelector("#generatedImageZoomPreview");
      if (!(preview instanceof HTMLImageElement)) return;

      preview.src = src;
      preview.alt = img.alt || "Preview gambar";
      openModal("cmImageZoomGenerated");
    });
  }
