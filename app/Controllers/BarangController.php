...
class BarangController
{
    ...
    public function index(Request $request): Response
    {
        ...
        try {
            ...
            $totalRows = (int) $pdo->query('SELECT COUNT(*) FROM `barang`')->fetchColumn();
            $displayColumns = $this->displayColumns($rows, $editableColumns);
            $rows = $this->decorateRowsWithHelpers($pdo, $rows, $displayColumns);
        } catch (Throwable) {
            toast_add('Gagal memuat data Barang.', 'error');
        }

        $html = app()->view()->render('barang/index', [
            'title' => 'Barang',
            'auth' => $auth,
            'activeMenu' => 'barang',
            'rows' => $rows,
    ...
    public function datatable(Request $request): Response
    {
        try {
            ...
            $countTotal = (int) $pdo->query('SELECT COUNT(*) FROM `barang`')->fetchColumn();
            if ($whereSql === '') {
                $countFiltered = $countTotal;
            } else {
                $countStmt = $pdo->prepare('SELECT COUNT(*) FROM `barang`' . $whereSql);
            ...
            $sql = 'SELECT * FROM `barang`' . $whereSql . ' ORDER BY `' . $orderColumn . '` ' . $orderDir . ' LIMIT :limit OFFSET :offset';
        ...
        try {
            ...
            foreach ($columns as $column) {
                ...
                if ($inputType === 'file' && $this->isImageColumnName($name)) {
                    $uploadResult = $this->handleBarangImageUpload($pdo, $name, $auth, $authId);
            ...
            if (stripos($e->getMessage(), 'upload') !== false || stripos($e->getMessage(), 'gambar') !== false) {
                ...
                toast_add('Gagal menambahkan data Barang.', 'error');
            }
    ...
    public function update(Request $request, string $id): Response
    {
        ...
        try {
            ...
            $sql = 'UPDATE `barang` SET ' . implode(', ', $setParts) . ' WHERE id = :id';
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
            ...
            toast_add('Data Barang berhasil diperbarui.', 'success');
        } catch (Throwable $e) {
            if (stripos($e->getMessage(), 'upload') !== false || stripos($e->getMessage(), 'gambar') !== false) {
                toast_add($e->getMessage(), 'error');
            } else {
                toast_add('Gagal memperbarui data Barang.', 'error');
            }
            $this->rollbackUploadedFiles($pdo ?? null, $createdFileIds, $createdAbsolutePaths);
            return Response::redirect('/barang');
        }
    ...
    private function editableColumns(\PDO $pdo): array
    {
        ...
        if ($rows === []) {
            try {
                $descRows = $pdo->query('DESCRIBE `barang`')->fetchAll();
        ...
        foreach ($rows as $index => $row) {
            ...
            foreach ($imageColumns as $column) {
                ...
                if ($rawValue !== '' && ctype_digit($rawValue)) {
                    $resolvedValue = (string) ($filemanagerPathMap[(int) $rawValue] ?? '');
                }
     ...
     */
    private function handleBarangImageUpload(\PDO $pdo, string $fieldName, array $auth, int $authId): array
    {
        ...
        $baseStorage = app()->basePath('storage/filemanager/barang/' . $roleSegment . '/' . $userSegment);
        if (!is_dir($baseStorage) && !mkdir($baseStorage, 0775, true) && !is_dir($baseStorage)) {
            throw new RuntimeException('Gagal membuat direktori upload gambar barang.');
        }

        $storedName = 'barang_' . date('YmdHis') . '_' . bin2hex(random_bytes(8)) . '.' . $extension;
        $targetPath = $baseStorage . DIRECTORY_SEPARATOR . $storedName;
        if (!move_uploaded_file($tmp, $targetPath)) {
            throw new RuntimeException('Gagal menyimpan gambar barang ke storage.');
        }

        $relativePath = 'filemanager/barang/' . $roleSegment . '/' . $userSegment . '/' . $storedName;
        ...
        if ($fileId <= 0) {
            @unlink($targetPath);
            throw new RuntimeException('Gagal menyimpan metadata gambar barang.');
        }
