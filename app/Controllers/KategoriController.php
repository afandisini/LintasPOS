...
class KategoriController
{
    public function index(Request $request): Response
    {
        ...
        try {
            ...
            $totalRows = (int) $pdo->query('SELECT COUNT(*) FROM `kategori`')->fetchColumn();
            $displayColumns = $this->displayColumns($rows, $editableColumns);
        } catch (Throwable) {
            toast_add('Gagal memuat data Kategori.', 'error');
        }

        $html = app()->view()->render('kategori/index', [
            'title' => 'Kategori',
            'auth' => $auth,
            'activeMenu' => 'kategori',
            'rows' => $rows,
    ...
    public function datatable(Request $request): Response
    {
        try {
            ...
            $countTotal = (int) $pdo->query('SELECT COUNT(*) FROM `kategori`')->fetchColumn();
            if ($whereSql === '') {
                $countFiltered = $countTotal;
            } else {
                $countStmt = $pdo->prepare('SELECT COUNT(*) FROM `kategori`' . $whereSql);
            ...
            $sql = 'SELECT * FROM `kategori`' . $whereSql . ' ORDER BY `' . $orderColumn . '` ' . $orderDir . ' LIMIT :limit OFFSET :offset';
    ...
    public function store(Request $request): Response
    {
        try {
            ...
            $stmt = $pdo->prepare('INSERT INTO `kategori` (' . $columnSql . ') VALUES (' . $valueSql . ')');
            $stmt->execute($params);
            toast_add('Data Kategori berhasil ditambahkan.', 'success');
        } catch (Throwable) {
            toast_add('Gagal menambahkan data Kategori.', 'error');
        }
    ...
    public function update(Request $request, string $id): Response
    {
        ...
        try {
            ...
            $sql = 'UPDATE `kategori` SET ' . implode(', ', $setParts) . ' WHERE id = :id';
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
            toast_add('Data Kategori berhasil diperbarui.', 'success');
        } catch (Throwable) {
            toast_add('Gagal memperbarui data Kategori.', 'error');
        }
    ...
    public function destroy(Request $request, string $id): Response
    {
        ...
        try {
            ...
            toast_add('Data Kategori berhasil dihapus.', 'success');
        } catch (Throwable) {
            toast_add('Gagal menghapus data Kategori.', 'error');
        }
    ...
    private function editableColumns(\PDO $pdo): array
    {
        ...
        if ($rows === []) {
            try {
                $descRows = $pdo->query('DESCRIBE `kategori`')->fetchAll();
