...
class SatuanController
{
    public function index(Request $request): Response
    {
        ...
        try {
            ...
            $stmt = $pdo->query('SELECT * FROM `satuan` ORDER BY id DESC');
            $rows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
            if (!is_array($rows)) {
                $rows = [];
            }
            $rows = $this->decorateRowsWithRelations($rows, $relations);
            $totalRows = (int) $pdo->query('SELECT COUNT(*) FROM `satuan`')->fetchColumn();
            $displayColumns = $this->displayColumns($rows, $editableColumns);
        } catch (Throwable) {
            toast_add('Gagal memuat data Satuan.', 'error');
        }

        $html = app()->view()->render('satuan/index', [
            'title' => 'Satuan',
            'auth' => $auth,
            'activeMenu' => 'satuan',
            'rows' => $rows,
    ...
    public function datatable(Request $request): Response
    {
        try {
            ...
            $countTotal = (int) $pdo->query('SELECT COUNT(*) FROM `satuan`')->fetchColumn();
            if ($whereSql === '') {
                $countFiltered = $countTotal;
            } else {
                $countStmt = $pdo->prepare('SELECT COUNT(*) FROM `satuan`' . $whereSql);
            ...
            $sql = 'SELECT * FROM `satuan`' . $whereSql . ' ORDER BY `' . $orderColumn . '` ' . $orderDir . ' LIMIT :limit OFFSET :offset';
            ...
            return Response::json([
                ...
                'recordsFiltered' => 0,
                'data' => [],
                'error' => 'Gagal memuat data Satuan.',
            ], 500);
        }
    }
    ...
    public function store(Request $request): Response
    {
        try {
            ...
            if ($columns === []) {
                ...
                return Response::redirect('/satuan');
            }
            ...
            $stmt = $pdo->prepare('INSERT INTO `satuan` (' . $columnSql . ') VALUES (' . $valueSql . ')');
            $stmt->execute($params);
            toast_add('Data Satuan berhasil ditambahkan.', 'success');
        } catch (Throwable) {
            toast_add('Gagal menambahkan data Satuan.', 'error');
        }
    ...
    public function update(Request $request, string $id): Response
    {
        ...
        try {
            ...
            $sql = 'UPDATE `satuan` SET ' . implode(', ', $setParts) . ' WHERE id = :id';
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
            toast_add('Data Satuan berhasil diperbarui.', 'success');
        } catch (Throwable) {
            toast_add('Gagal memperbarui data Satuan.', 'error');
        }
    ...
    public function destroy(Request $request, string $id): Response
    {
        ...
        try {
            $pdo = Database::connection();
            $stmt = $pdo->prepare('DELETE FROM `satuan` WHERE id = :id');
            $stmt->execute(['id' => $recordId]);
            toast_add('Data Satuan berhasil dihapus.', 'success');
        } catch (Throwable) {
            toast_add('Gagal menghapus data Satuan.', 'error');
        }
    ...
    private function editableColumns(\PDO $pdo): array
    {
        ...
        if ($rows === []) {
            try {
                $descRows = $pdo->query('DESCRIBE `satuan`')->fetchAll();
