...
class DiskonController
{
    public function index(Request $request): Response
    {
        ...
        try {
            ...
            $stmt = $pdo->query('SELECT * FROM `diskon` ORDER BY id DESC');
            $rows = $stmt->fetchAll(\PDO::FETCH_ASSOC);
            if (!is_array($rows)) {
                $rows = [];
            }
            $rows = $this->decorateRowsWithRelations($rows, $relations);
            $totalRows = (int) $pdo->query('SELECT COUNT(*) FROM `diskon`')->fetchColumn();
            $displayColumns = $this->displayColumns($rows, $editableColumns);
            $rows = $this->decorateRowsWithHelpers($pdo, $rows, $displayColumns);
        } catch (Throwable) {
            toast_add('Gagal memuat data Diskon.', 'error');
        }

        $html = app()->view()->render('diskon/index', [
            'title' => 'Diskon',
            'auth' => $auth,
            'activeMenu' => 'diskon',
            'rows' => $rows,
    ...
    public function datatable(Request $request): Response
    {
        try {
            ...
            $countTotal = (int) $pdo->query('SELECT COUNT(*) FROM `diskon`')->fetchColumn();
            if ($whereSql === '') {
                $countFiltered = $countTotal;
            } else {
                $countStmt = $pdo->prepare('SELECT COUNT(*) FROM `diskon`' . $whereSql);
            ...
            $sql = 'SELECT * FROM `diskon`' . $whereSql . ' ORDER BY `' . $orderColumn . '` ' . $orderDir . ' LIMIT :limit OFFSET :offset';
            ...
            return Response::json([
                ...
                'recordsFiltered' => 0,
                'data' => [],
                'error' => 'Gagal memuat data Diskon.',
            ], 500);
        }
    }
    ...
    public function store(Request $request): Response
    {
        try {
            ...
            if ($columns === []) {
                ...
                return Response::redirect('/diskon');
            }
    ...
    private function editableColumns(\PDO $pdo): array
    {
        ...
        if ($rows === []) {
            try {
                $descRows = $pdo->query('DESCRIBE `diskon`')->fetchAll();
