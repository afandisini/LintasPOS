...
class SupplierController
{
    public function index(Request $request): Response
    {
        ...
        try {
            ...
            $totalRows = (int) $pdo->query('SELECT COUNT(*) FROM `supplier`')->fetchColumn();
            $displayColumns = $this->displayColumns($rows, $editableColumns);
            $rows = $this->decorateRowsWithHelpers($pdo, $rows, $displayColumns);
        } catch (Throwable) {
            toast_add('Gagal memuat data Supplier.', 'error');
        }

        $html = app()->view()->render('supplier/index', [
            'title' => 'Supplier',
            'auth' => $auth,
            'activeMenu' => 'supplier',
            'rows' => $rows,
    ...
    public function datatable(Request $request): Response
    {
        try {
            ...
            $countTotal = (int) $pdo->query('SELECT COUNT(*) FROM `supplier`')->fetchColumn();
            if ($whereSql === '') {
                $countFiltered = $countTotal;
            } else {
                $countStmt = $pdo->prepare('SELECT COUNT(*) FROM `supplier`' . $whereSql);
            ...
            $sql = 'SELECT * FROM `supplier`' . $whereSql . ' ORDER BY `' . $orderColumn . '` ' . $orderDir . ' LIMIT :limit OFFSET :offset';
    ...
    public function store(Request $request): Response
    {
        try {
            ...
            $stmt->execute($params);
            toast_add('Data Supplier berhasil ditambahkan.', 'success');
        } catch (Throwable) {
            toast_add('Gagal menambahkan data Supplier.', 'error');
        }
    ...
    public function update(Request $request, string $id): Response
    {
        ...
        try {
            ...
            $sql = 'UPDATE `supplier` SET ' . implode(', ', $setParts) . ' WHERE id = :id';
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
            toast_add('Data Supplier berhasil diperbarui.', 'success');
        } catch (Throwable) {
            toast_add('Gagal memperbarui data Supplier.', 'error');
        }
    ...
    public function destroy(Request $request, string $id): Response
    {
        ...
        try {
            ...
            toast_add('Data Supplier berhasil dihapus.', 'success');
        } catch (Throwable) {
            toast_add('Gagal menghapus data Supplier.', 'error');
        }
    ...
    private function editableColumns(\PDO $pdo): array
    {
        ...
        if ($rows === []) {
            try {
                $descRows = $pdo->query('DESCRIBE `supplier`')->fetchAll();
